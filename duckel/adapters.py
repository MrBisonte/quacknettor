            ...
            if evolution == "fail":
                ...
                for col, dtype in missing_cols.items():
                    try:
                        ...
                        raise AdapterError(
                            f"Schema evolution failed while adding column '{col}': {e}"
                        ) from e
            else:
...
class PostgresTargetAdapter(TargetAdapter):
    ...
    def attach(self, con):
        ...
        try:
            ...
            if any(
                kw in error_msg
                for kw in ["authentication failed", "password", "role", "permission denied"]
            ):
                raise AdapterError(
                ...
                kw in error_msg
                for kw in ["could not connect", "timeout", "host", "connection refused", "no route"]
            ):
                raise AdapterError(
...
class SnowflakeTargetAdapter(TargetAdapter):
    ...
    def attach(self, con):
        ...
        try:
            ...
            if any(
                kw in error_msg
                for kw in ["authentication", "password", "incorrect", "user", "login"]
            ):
                raise AdapterError(
                ...
                kw in error_msg
                for kw in ["could not connect", "timeout", "account", "host", "network"]
            ):
                raise AdapterError(
